...
import { TwoActionModalType } from "./TwoActionModal.specs"
import useEvent from "react-use-event-hook"

const TwoActionModal: TwoActionModalType = (props) => {
  const cancelButtonRef = useRef(null)

  const handleClose = useEvent((e) => {
    props.onClose?.(e)
  })
  const handleOk = useEvent((e) => {
    props.onOk?.(e)
  })
  const handleCancel = useEvent((e) => {
    props.onCancel?.(e)
  })
  const Icon = props.icon || CheckOutline
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              <div>
                ...
                >
                  <Icon
                    className={"h-6 w-6 " + "text-" + color + "-700"}
                    aria-hidden="true"
                  />
                </div>
